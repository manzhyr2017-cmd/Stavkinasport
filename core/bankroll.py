...
class BankrollManager:
    ...
    def __init__(self, initial_bankroll: float = None):
        ...
        # Persistence
        self.data_file = "data/bankroll.json"
        self._load_state()
    ...
    def _load_state(self):
        ...
        try:
            ...
            self._losing_streak = data.get("losing_streak", 0)
            self._is_stopped = data.get("is_stopped", False)
            self._stop_reason = data.get("stop_reason", "")
            
            raw_history = data.get("bet_history", [])
            self.bet_history = []
            ...
            logger.info(f"💰 Bankroll state loaded: {self.bankroll:.0f}₽")
        except Exception as e:
    ...
    def check_stop_conditions(self) -> bool:
        ...
        if self.bankroll < self.initial_bankroll * self.cfg.BANKRUPTCY_THRESHOLD:
            self._stop(f"Bankroll critical: {self.bankroll:.0f}₽")
            return True
    ...
    def record_bet(self, signal_id: str, bet_type: str,
                   stake: float, odds: float, match_info: str = "") -> BetRecord:
        record = BetRecord(signal_id=signal_id, bet_type=bet_type,
                           stake=stake, odds=odds, match_info=match_info)
    ...
    def stats_telegram_message(self) -> str:
        ...
        return (
            ...
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"💰 <b>{s['bankroll']:.0f}₽</b> (старт: {s['initial']:.0f}₽)\n"
            f"📈 Пик: {s['peak']:.0f}₽  {dd}\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            ...
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"📅 Сегодня: {s['daily_pnl']:+.0f}₽\n"
            f"📆 Неделя: {s['weekly_pnl']:+.0f}₽\n"
        )
