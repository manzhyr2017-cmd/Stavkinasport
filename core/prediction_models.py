...
class DixonColesModel:
    ...
    def _neg_log_likelihood(self, params_flat: np.ndarray,
        ...
        for match in matches:
            ...
            # Если xG доступна и use_xg=True, используем её как "гол" (сглаженно)
            if use_xg and "home_xg" in match and match["home_xg"] is not None:
                x = match["home_xg"]
                y = match["away_xg"]
            else:
                x = match["home_goals"]
    ...
    def fit(self, matches: List[dict]) -> "DixonColesModel":
        ...
        result = minimize(
            ...
            x0,
            args=(matches, team_index, model_config.USE_XG_FOR_TRAINING),
            method="SLSQP",
            bounds=bounds,
    ...
    def save(self, path: str = "models/dc_params.json"):
        import json
        import os
        try:
            ...
            with open(path, "w") as f:
                json.dump(data, f)
            logger.info(f"Dixon-Coles params saved to {path}")
        except Exception as e:
            logger.error(f"Error saving DC params: {e}")

    def load(self, path: str = "models/dc_params.json"):
        import json
        import os
        if os.path.exists(path):
            try:
                ...
                self.rho = data["rho"]
                logger.info(f"Dixon-Coles params loaded from {path} ({len(self.teams)} teams)")
                return True
            except Exception as e:
                logger.error(f"Error loading DC params: {e}")
        return False
...
class EloRatingSystem:
    ...
    def save(self, path: str = "models/elo_ratings.json"):
        import json
        import os
        try:
            os.makedirs(os.path.dirname(path), exist_ok=True)
            ...
            logger.info(f"Elo ratings saved to {path}")
        except Exception as e:
            logger.error(f"Error saving Elo ratings: {e}")

    def load(self, path: str = "models/elo_ratings.json"):
        import json
        import os
        if os.path.exists(path):
            try:
                ...
                logger.info(f"Elo ratings loaded from {path} ({len(self.ratings)} teams)")
                return True
            except Exception as e:
...
class ProbabilityCalibration:
    """
    Калибровка вероятностей с использованием Isotonic Regression.
    
    Исследование arxiv.org/pdf/2303.06021 показало, что калибровка 
    важнее accuracy для ROI в беттинге.
    """
    ...
    def __init__(self, method: str = "isotonic"):
        try:
            ...
            logger.warning("sklearn.calibration not installed. Calibration will be disabled.")
    ...
    def fit(self, y_true: np.ndarray, y_prob: np.ndarray):
        ...
        Args:
            y_true: массив истинных меток (0=Home, 1=Draw, 2=Away)
            y_prob: матрица (N, 3) предсказанных вероятностей
        """
        if not self.calibrators:
            return

        if len(y_true) < model_config.CALIBRATION_MIN_SAMPLES:
            logger.warning(f"Too few samples for calibration: {len(y_true)}")
            return

        for i, outcome in enumerate(["home", "draw", "away"]):
            y_binary = (y_true == i).astype(int)
            self.calibrators[outcome].fit(y_prob[:, i], y_binary)
        
        self._fitted = True
        logger.info("Probability calibrators fitted using Isotonic Regression")

    def calibrate(self, probs: dict) -> dict:
        """Применить калибровку к предсказанию"""
        if not self._fitted:
            return probs
        ...
        for outcome in ["home", "draw", "away"]:
            if outcome in probs:
                raw_p = probs[outcome]
                cal_p = self.calibrators[outcome].predict([raw_p])[0]
                calibrated[outcome] = cal_p
    ...
    def save(self, path: str = "models/calibrator.pkl"):
        ...
        try:
            ...
            with open(path, "wb") as f:
                pickle.dump({"calibrators": self.calibrators, "fitted": self._fitted}, f)
            logger.info(f"Calibrator saved to {path}")
        except Exception as e:
            logger.error(f"Error saving calibrator: {e}")

    def load(self, path: str = "models/calibrator.pkl"):
        import pickle
        import os
        if os.path.exists(path):
            try:
...
class EnsemblePredictor:
    ...
    def __init__(
        ...
        calibrator: Optional[ProbabilityCalibration] = None,
    ):
        self.dc = dixon_coles
        self.elo = elo
        ...
        if model_config.USE_CATBOOST:
            try:
                from core.ml_pipeline import CatBoostPipeline
                self.catboost_pipeline = CatBoostPipeline()
                self.catboost_pipeline.load()
            except ImportError:
                logger.warning("CatBoostPipeline not available")
            except Exception as e:
    ...
    def predict(self, home: str, away: str,
        ...
        # 4. CatBoost
        if self.catboost_pipeline:
            # Logic to extract features would go here, simplified for now
            # cb_pred = self.catboost_pipeline.predict(...)
            pass
        ...
        # 5. Calibration
        if self.calibrator:
            ensemble = self.calibrator.calibrate(ensemble)
