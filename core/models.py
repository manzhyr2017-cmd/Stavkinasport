...
# Маппинг для российского рынка (The Odds API -> RU Brands)
RU_BOOKMAKER_MAPPING = {
    "1xBet": "1xBet / Melbet",
    "Marathon Bet": "Марафон",
    "Pinnacle": "Pinnacle (Профи)",
    "Unibet": "BetBoom / Unibet",
    "Betfair": "Betfair (Биржа)",
    "William Hill": "Winline (через WH)",
    "Betsson": "Leon (через Betsson)",
    "888sport": "PARI (через 888)",
    "Betclic (FR)": "Fonbet (через FR)",
}
...
class ValueSignal:
    ...
    lstm_prediction: Optional[dict] = None  # NEW: LSTM forecast results
    status: SignalStatus = SignalStatus.PENDING
    analysis: str = ""           # NEW: Reasoning for the bet
    created_at: datetime = field(default_factory=datetime.utcnow)

    def to_telegram_message(self) -> str:
        if not self.match:
            return "⚠️ Данные матча отсутствуют"
        m = self.match
        ...
        # Перевод исхода
        outcome_ru = self.outcome.value.upper()
        if self.outcome == BetOutcome.HOME:
            outcome_ru = f"П1 (Победа {m.home_team})"
        elif self.outcome == BetOutcome.AWAY:
            outcome_ru = f"П2 (Победа {m.away_team})"
        elif self.outcome == BetOutcome.DRAW:
            outcome_ru = "Ничья (X)"

        # Индикатор "шарпа" и приоритетности
        sharp = "🎯 Sharp Line" if self.sharp_agrees else "📊 Market Line"
        
        # Проверка приоритетного БК и маппинг
        display_bm = RU_BOOKMAKER_MAPPING.get(self.bookmaker_name, self.bookmaker_name)
        
        # Список для пометки "звездой" (включая RU маппинги)
        ru_priority = [
            "1xBet", "Melbet", "Марафон", "Pinnacle", "BetBoom", 
            "Winline", "Fonbet", "Leon", "PARI", "Лига Ставок"
        ]
        is_priority = any(p in display_bm for p in ru_priority)
        priority_star = " ⭐" if is_priority else ""

        # Аналитика
        analysis_block = ""
        if self.analysis:
            analysis_block = f"🧠 <b>АНАЛИЗ AI:</b>\n{self.analysis}\n━━━━━━━━━━━━━━━━━━━━━\n"

        return (
            f"🎯 <b>ВАЛУЙНАЯ СТАВКА</b> {self.confidence_level.value}\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"⚽ <b>{m.home_team}</b> — <b>{m.away_team}</b>\n"
            f"🏆 {m.league} | 🕐 {m.commence_time:%d.%m %H:%M}\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"{emoji} Прогноз: <b>{outcome_ru}</b>\n"
            f"📈 Коэффициент: <b>{self.bookmaker_odds:.2f}</b>{priority_star}\n"
            f"🏢 Букмекер: <b>{display_bm}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"{analysis_block}"
            f"🧠 Вероятность: <b>{self.model_probability:.1%}</b> ({self.model_count} мод.)\n"
            f"💎 Перевес (Edge): <b>+{self.edge:.1%}</b>\n"
            f"📊 {sharp} | Маржа: {m.overround:.1%}\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"💰 Рекомендуемая ставка: <b>{self.stake_amount:.0f}₽</b>\n"
            f"📈 Доля от банка: <b>{self.kelly_stake:.1%}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"🆔 <code>#{self.id}</code>"
        )
...
class ExpressBet:
    ...
    stake_amount: float = 0.0
    status: SignalStatus = SignalStatus.PENDING
    analysis: str = ""  # NEW: Explanation for why this express was built
    created_at: datetime = field(default_factory=datetime.utcnow)
    ...
    def to_telegram_message(self) -> str:
        ...
        for i, leg in enumerate(self.legs, 1):
            m = leg.signal.match
            if m:
                # Русское описание исхода
                outcome = leg.signal.outcome
                outcome_ru = outcome.value.upper()
                if outcome == BetOutcome.HOME:
                    outcome_ru = f"П1 ({m.home_team})"
                elif outcome == BetOutcome.AWAY:
                    outcome_ru = f"П2 ({m.away_team})"
                elif outcome == BetOutcome.DRAW:
                    outcome_ru = "Ничья (X)"
                
                legs_text += (
                    f"  {i}. {m.home_team} — {m.away_team}\n"
                    f"     🎯 <b>{outcome_ru}</b> "
                    f"@{leg.odds:.2f}\n"
                )
        ...
        return (
            f"🔥 <b>ЭКСПРЕСС ИЗ ОРДИНАРОВ ({len(self.legs)} событий)</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"{legs_text}"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"{analysis_block}"
            f"📊 Общий коэффициент: <b>{self.total_odds:.2f}</b>\n"
            f"🧠 Вероятность: <b>{self.combined_probability:.1%}</b>"
            f"{corr_note}\n"
            f"💎 Ожидание (EV): <b>{self.adjusted_ev:+.1%}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"💰 Ставка: <b>{self.stake_amount:.0f}₽</b>\n"
            f"🎯 Потенциальный выигрыш: <b>{self.potential_win:.0f}₽</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"🆔 <code>#{self.id}</code>"
        )
...
class SystemBet:
    ...
    def to_telegram_message(self) -> str:
        ...
        return (
            ...
            f"💰 <b>{self.stake_per_combo:.0f}₽</b> × "
            f"{self.num_combinations} = "
            f"<b>{self.total_stake:.0f}₽</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"🆔 <code>#{self.id}</code>"
        )
