...
├── core/
│   ├── prediction_models.py    # Dixon-Coles + Elo + Ensemble Predictor
│   ├── value_engine.py         # 4x overround removal + corr. expresses
│   ├── bankroll.py             # Adaptive Kelly + multi stop-loss
│   ├── signal_generator.py     # Orchestrator
│   └── models.py               # Domain models (Match, Signal, Express, System)
├── data/
...
- [x] **V2.0** — Dixon-Coles, Elo, Shin, Adaptive Kelly, Correlation Express
- [ ] **V2.1** — CatBoost training pipeline + isotonic calibration
- [ ] **V2.2** — NLP (травмы из новостей), xG features from FBRef
- [ ] **V2.3** — Live WebSocket monitoring + LSTM line prediction
- [ ] **V2.4** — Web dashboard (FastAPI + React)
